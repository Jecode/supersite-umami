-- Create Event
CREATE TABLE supersite.website_event
(
    website_id UUID,
    session_id UUID,
    visit_id UUID,
...
CREATE TABLE supersite.event_data
(
    website_id UUID,
    session_id UUID,
    event_id UUID,
...
CREATE TABLE supersite.session_data
(
    website_id UUID,
    session_id UUID,
    data_key String,
...
-- stats hourly
CREATE TABLE supersite.website_event_stats_hourly
(
    website_id UUID,
    session_id UUID,
    visit_id UUID,
...
CREATE MATERIALIZED VIEW supersite.website_event_stats_hourly_mv
TO supersite.website_event_stats_hourly
AS
SELECT
    website_id,
    session_id,
...
-- projections
ALTER TABLE supersite.website_event 
ADD PROJECTION website_event_url_path_projection (
SELECT * ORDER BY toStartOfDay(created_at), website_id, url_path, created_at
);

ALTER TABLE supersite.website_event MATERIALIZE PROJECTION website_event_url_path_projection;
...
ALTER TABLE supersite.website_event MATERIALIZE PROJECTION website_event_referrer_domain_projection;

-- revenue
CREATE TABLE supersite.website_revenue
(
    website_id UUID,
    session_id UUID,
    event_id UUID,
...
CREATE MATERIALIZED VIEW supersite.website_revenue_mv
TO supersite.website_revenue
AS
SELECT DISTINCT
    ed.website_id,
    ed.session_id,
    ...
    ed.created_at
FROM supersite.event_data ed
JOIN (SELECT event_id, string_value as currency
        FROM supersite.event_data
